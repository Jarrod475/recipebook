...
function Edit(props){
   ...
   function setnewIngredients(index,value){
    ...
    //this loads the recipe data into the component so that we can make the edits.
    useEffect(()=>{
        console.log("recipe is : ", props.currentRecipe);
        setName(props.currentRecipe.name);
        setIngredients(props.currentRecipe.ingredients)
        setInstruction(props.currentRecipe.instructions)
    },[]);

    async function submitRecipe() {
        await axios.post('http://localhost:5000/edit',{data :{recipeID : props.currentRecipe.id , recipe : recipe}})
        .then(function (response){
            console.log(response);
            alert("Recipe has been changed!");
          })
        .catch(error => {
